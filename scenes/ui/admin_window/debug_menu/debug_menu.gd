...
func _on_viewer_joined(display_name: String):
	if preferences.active_viewers.find(display_name) > -1:
		return
	preferences.active_viewers.append(display_name)
	if preferences.restart_with_active_viewers:
		save_preferences()

func _on_viewer_left(display_name: String):
	var index = preferences.active_viewers.find(display_name)
	if index < 0:
		return
	preferences.active_viewers.remove_at(index)
	if preferences.restart_with_active_viewers:
		save_preferences()
...
class DebugPreferences:
	...
	static func from_dictionary(dict: Dictionary) -> DebugPreferences:
		...
		if preferences.restart_with_active_viewers:
			preferences.active_viewers.append_array(dict["active_viewers"])
		return preferences
